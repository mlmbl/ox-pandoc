...
the like is removed from =ox-pandoc=. This is to avoid conflict with
pandoc's own numbering. It is recommended to use pandoc for numbering.

Since this is a breaking change, you may pin your =ox-pandoc= version to
the last one that included numbering of figures if you require the old
behaviour. The last numbered version of =ox-pandoc= with numbering is
*v1.2*.
* History and People
